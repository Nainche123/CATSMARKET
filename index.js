...
const ADMIN_IDS = String(process.env.ADMIN_DISCORD_IDS || "").split(",").map(s=>s.trim()).filter(Boolean);
...
function admin(req){const u=currentUser(req);return !!u&&ADMIN_IDS.includes(String(u.discord_id))}
function findUser(discordId){return data.users.find(u=>String(u.discord_id)===String(discordId))}
...
app.get("/auth/discord",(req,res)=>{if(!process.env.DISCORD_CLIENT_ID||!process.env.DISCORD_CLIENT_SECRET)return res.status(503).send("Discord OAuth 설정이 필요합니다.");const redirectUri=process.env.DISCORD_REDIRECT_URI||`${BASE_URL}/auth/discord/callback`;const qs=new URLSearchParams({client_id:process.env.DISCORD_CLIENT_ID,redirect_uri:redirectUri,response_type:"code",scope:"identify",prompt:"consent"});res.redirect(`https://discord.com/oauth2/authorize?${qs.toString()}`)});
app.get("/auth/discord/callback",async(req,res)=>{try{const code=String(req.query.code||"");if(!code)return res.status(400).send("인증 코드가 없습니다.");const redirectUri=process.env.DISCORD_REDIRECT_URI||`${BASE_URL}/auth/discord/callback`;const tokenR=await fetch("https://discord.com/api/v10/oauth2/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({client_id:process.env.DISCORD_CLIENT_ID,client_secret:process.env.DISCORD_CLIENT_SECRET,grant_type:"authorization_code",code,redirect_uri:redirectUri})});if(!tokenR.ok)throw new Error("Discord token exchange failed");const token=await tokenR.json();const meR=await fetch("https://discord.com/api/v10/users/@me",{headers:{Authorization:`Bearer ${token.access_token}`}});if(!meR.ok)throw new Error("Discord profile fetch failed");const me=await meR.json();const u=upsertUser(me);req.session.user={id:u.id,discord_id:u.discord_id,username:u.username,avatar:u.avatar};res.redirect("/")}catch(e){console.error(e);res.status(500).send("Discord 로그인 처리 중 오류가 발생했습니다.")}});
...
app.get("/api/orders",(req,res)=>{const u=currentUser(req);if(!u)return res.status(401).json({error:"Discord 로그인이 필요합니다."});res.json(data.orders.filter(o=>o.discord_id===u.discord_id).slice(0,50))});
